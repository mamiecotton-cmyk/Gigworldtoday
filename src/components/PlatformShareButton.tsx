...
export default function PlatformShareButton({ platformName, slug }: Props) {
  const [open, setOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const ref = useRef<HTMLDivElement>(null);

  const url = `https://gigworldtoday.com/platforms/${slug}`;
  const text = `Check out ${platformName} on GigWorldToday — compare gig platforms and find your best earning opportunity.`;
  ...
  const shareOptions = [
    ...
    },
    {
      label: "Reddit",
      icon: <RedditIcon />,
      action: () => window.open(`https://www.reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${platformName} — Gig Platform Review`)}`, "_blank"),
    },
    {
      label: "WhatsApp",
      icon: <WhatsAppIcon />,
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((o) => !o)}
        className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-200 bg-white text-gray-600 hover:text-teal-600 hover:border-teal-300 transition-colors text-sm font-medium shadow-sm"
      >
